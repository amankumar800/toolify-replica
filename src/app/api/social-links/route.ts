...
import type { SocialLinkRow, SocialLinksResponse } from '@/lib/supabase/types';

/**
 * GET /api/social-links
 * 
 * Fetches active social links for the footer.
  ...
  try {
    ...
    (data as SocialLinkRow[]).forEach((link) => {
      if (link.url && link.url.trim() !== '') {
        response[link.platform as keyof SocialLinksResponse] = link.url;
      }
    });
