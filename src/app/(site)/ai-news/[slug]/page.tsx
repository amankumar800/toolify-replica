...
export async function generateMetadata(
    ...
    const news = await NewsService.getNewsBySlug(params.slug, { preview: isPreview });
...
export default async function NewsDetailPage(
    ...
    const { slug } = params;
    const isPreview = searchParams.preview === 'true';
    const news = await NewsService.getNewsBySlug(slug, { preview: isPreview });
    ...
    return (
        <>
            {/* Draft Preview Banner - Requirements: 18.4 */}
            {isPreview && isDraft && (
                <DraftPreviewBanner
                    contentType="news"
                    contentId={news.id}
            ...
            <div className={`container mx-auto px-4 py-8 ${isPreview && isDraft ? 'pt-20' : ''}`}>
                {/* Breadcrumb / Back */}
                <div className="mb-6">
                    <Link
                        href="/ai-news"
